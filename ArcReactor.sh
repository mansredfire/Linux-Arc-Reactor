...
install_python_tools() {
    ...
    if git clone https://github.com/dark-warlord14/LinkFinder.git "$INSTALL_DIR/linkfinder" 2>&1 | tee -a "$LOG_FILE"; then
    ...
    if git clone https://github.com/m4ll0k/SecretFinder.git "$INSTALL_DIR/secretfinder" 2>&1 | tee -a "$LOG_FILE"; then
    ...
    if git clone https://github.com/nsonaniya2010/SubDomainizer.git "$INSTALL_DIR/subdomainizer" 2>&1 | tee -a "$LOG_FILE"; then
    ...
    if git clone https://github.com/swisskyrepo/GraphQLmap.git "$INSTALL_DIR/graphqlmap" 2>&1 | tee -a "$LOG_FILE"; then
    ...
    if git clone https://github.com/codingo/NoSQLMap.git "$INSTALL_DIR/nosqlmap" 2>&1 | tee -a "$LOG_FILE"; then
    ...
    if git clone https://github.com/swisskyrepo/SSRFmap.git "$INSTALL_DIR/ssrfmap" 2>&1 | tee -a "$LOG_FILE"; then
